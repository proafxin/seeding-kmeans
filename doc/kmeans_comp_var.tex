...
\begin{document}
	...
	\maketitle
		\begin{abstract}%
			$k$-Means is a popular clustering algorithm that reduces sum of minimum distances from data points to the centers. In this paper, we introduce a new way of initializing the centers. We follow standard practices for data pre-processing and show comparison of different initialization for $k$-Means centers in terms of inertia. Empirical evidence shows that our variance based initialization performs better than Lloyd, Ostrovsky-Rabani-Schulman-Swamy (ORSS) and $k$-Means++ algorithm in terms of inertia.
		\end{abstract}
	...
	Widely regarded as the most popular clustering techniques, $k$-Means remains a humble interesting topic in machine learning as well as computational geometry. Roughly the problem is: given a set of points $\X$ in $\mathbb{R}^d$. Find a set of centers $\mathcal{C}$ such that the function \textit{inertia}
		...
		\end{align*}
	is minimum where $\|\cdot\|$ is the $L_2$ norm\footnote{Simply put, it is the distance between the center $\c$ and point $\x$ or the magnitude of the vector $\c-\x$}.
	...
	There has been multiple surveys on $k$-Means in the literature. Probably the most relevant work in this regard is done by \cite{celebi}. However, most of the algorithms used for comparison are practically not used very much, as also noted by the authors themselves that $k$-Means++ and its greedy version work better than most. They also mention that probabilistic algorithms work better than deterministic ones. Moreover, another highly influential center initialization algorithm \cite{ostrovsky} was not considered in their experiments. There is no mention of Ostrovsky's algorithm in their paper whatsoever.
	...
	First we describe $k$-Means, Ostrovsky's and $k$-Means++ algorithm. Then we describe our proposed method of center initialization. For a set of points $S$ and a point $x$, we use $\min(\|x-S\|)$ to denote the minimum distance from $x$ to a point of $S$ i.e. $\min(\| x-S\|)=\min_{a\in S}(\| x-a\|)$.
	\subsection{Lloyd}
	Lloyd's algorithm is the simplest way of solving the $k$-Means problem.
		\begin{enumerate}[i]
			...
			\item For each $i$, $C_i$ is the set of points $\x\in\X$ such that $\|\x-c_i\|\leq \|\x-c_j\|$ for all $1\leq j\leq n$.
			\item Set $c_i=\frac{1}{|C_i|}\sum_{\x\in C_i}\x$.
	...
	ORSS algorithm \cite{ostrovsky} chooses two initial centers instead of one.
		\begin{enumerate}[i]
			\item Choose two points $\x,\y\in\X$ with probability proportional to $\|\x-\y\|^2$. Set $\C=\{\x,\y\}$.
			\item For already $i$ existing centers, choose a random point $\x\in\X$ with probability proportional to $\min(\| \x-\C\|^2)$.
	...
	Next lemma was proven in \cite[Lemma $3.2$]{ostrovsky}.
		\begin{lemma}
			If $\rho=\Omega(\epsilon^{\frac{2}{3}})$ for some real number $\epsilon$, then the first two centers lie in different cores with probability $1-O(p)$.
		\end{lemma}
	We will analyze the performance of this algorithm in terms of inertia. To be more specific, we will analyze the expected value of inertia since this is a probabilistic algorithm.
