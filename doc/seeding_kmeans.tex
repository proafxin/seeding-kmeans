...
\begin{document}
	...
	Before proceeding on to the main discussion, we would like to discuss a sensitive issue. The primary motivation behind our work is not simply improving \texttt{k-means}. \texttt{k-means++} is the most popular center initialization seeding method. It seems very justified given how simple it is and the theoretical as well as experimental results given in the original \texttt{k-means++} paper \citep{kmeans++} are very impressive. Even in the very popular scientific python package \texttt{scikit-learn} \citep{sklearn}, \texttt{k-means++} initialization is supported by default. Unfortunately, we believe neither theoretical nor experimental results shown in that paper are accurate. We will discuss our reasoning in section \eqref{sec:kmeans++vsorss}.
